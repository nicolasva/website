$(document).ready ->
  ...
  if exp_header_background_url.test(location_href) && exp_header_background_edit.test(location_href) == false 
    ...
    $.getJSON("/admin/header_backgrounds/"+header_backgrounds_uuid, (data) -> 
      ...
      $(".header_background").css("background", "url('"+data.background.background_image.url+" "+ if data.background.no_repeat is true then + 'no_repeat' else '' +"')") unless data.background.no_background_image
