  ...
  $(document).ready -> 
    ...
    if exp_menu_backgrounds_url.test(location_href) && exp_menu_background_edit.test(location_href) == false
      ...
      $.getJSON("/admin/menu_backgrounds/"+menu_backgrounds_uuid, (data) ->
        ...
        $(".menu_background").css("background", "url('"+data.background.background_image.url+" "+ if data.background.no_repeat is true then + 'no-repeat' else '' +"')") unless data.background.no_background_image
