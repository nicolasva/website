jQuery ->
  $(document).ready ->
    $.getJSON("/background_by_defaults", (data) -> 
      unless _.isNull(data)
        unless data.background.no_background_image
          $('.background_by_default').css("background", "url('"+data.background.background_image.url+" "+ if data.background.no_repeat is true then 'no_repeat' else '' +"')")
        else
    ...
    $.getJSON("/menu_backgrounds", (data) ->
      unless _.isNull(data)
        unless data.background.no_background_image
          $(".menu_color").css("background", "url('"+data.background.background_image.url+" "+ if data.background.no_repeat is true then 'no_repeat' else '' +"')")
        else
    ...
    $.getJSON("/sub_menu_backgrounds", (data) ->
      unless _.isNull(data)
        unless data.background.no_background_image
          $(".sub_menu").css("background", "url('"+data.background.background_image.url+" "+ if data.background.no_repeat is true then 'no_repeat' else '' +"')")
        else
